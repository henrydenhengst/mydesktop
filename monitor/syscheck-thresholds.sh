...
# Check of er kritieke fouten in de bash-omgeving of scripts staan (optioneel uit te breiden)
...
# Waarschuwing als SSH op poort 22 openstaat naar het internet zonder Fail2Ban (indien Fail2Ban ontbreekt)
...
# Waarschuwing als er cruciale updates klaarstaan (werkt op Debian/Ubuntu systemen met apt-check)
...
# Check op recente hardware/kernel crashes in dmesg (afgelopen uur)
...
# Optioneel: Temperatuur check indien 'sensors' aanwezig is
if has_cmd sensors; then
    HIGH_TEMP=$(sensors 2>/dev/null | grep -E '(Core 0|temp1)' | awk '{print $2}' | tr -d '+°C' | awk '{if($1 > 80) print $1}')
    if [ -n "$HIGH_TEMP" ]; then
        add_warning "CPU Temperatuur is kritiek hoog (>80°C): ${HIGH_TEMP}°C." "Controleer de koeling of cpu-frequentie via 'sensors' en 'upower'."
    fi
fi
...
# Controleer of er in de afgelopen 7 dagen (7 * 1440 min) een backup-archief of .bak is aangeraakt in de thuismap
if [ -z "$(find /home -maxdepth 3 -mtime -7 \( -name "*.tar.gz" -o -name "*.zip" -o -name "*.bak" \) 2>/dev/null)" ]; then
    add_warning "Geen recente backups of archieven (.tar.gz/.zip) gevonden uit de afgelopen 7 dagen." "Maak een handmatige backup met 'tar -czf backup.tar.gz folder/' of zet een 'rsync' sync-taak op."
fi
...
if [ -n "$WARNINGS" ]; then
    echo -e "⚠️  ${bold}LINUX BEST PRACTICE ALERTS — AANDACHT VEREIST${RESET}"
    ...
    # Alles is oké, wees volledig stil
    exit 0
fi
